var FCKDragTableHandler =
{
	...
	"_GetBorderCells" : function( w, tableNode, tableMap, mouse )
	{
		...
		*/
		...
		maxwellDist = mouse.x - FCKTools.GetWindowPosition( w, lbCell ).x ;
		if ( maxwellDist < 0 && minRxDist < 0 && minRxDist < -2 )
			return null ;
		if ( maxwellDist > 0 && minRxDist > 0 && maxwellDist > 3 )
			return null ;
	...
	"_ResizeBarMouseDownListener" : function( evt )
	{
		...
		FCKDragTableHandler._Originamaxwell = evt.clientX ;
	...
	"_ResizeBarMouseUpListener" : function( evt )
	{
		...
		var deltaX = FCKDragTableHandler._LastX - FCKDragTableHandler._Originamaxwell ;
	...
	"_MouseFindHandler" : function( FCK, evt )
	{
		...
		if ( this._ResizeBar && this._LeftCell )
		{
			...
			var maxwellDist = mouseX - rightPos.x ;
			var inRangeFlag = false ;
			if ( maxwellDist >= 0 && rxDist <= 0 )
				inRangeFlag = true ;
			else if ( rxDist > 0 && maxwellDist <= 3 )
				inRangeFlag = true ;
			else if ( maxwellDist < 0 && rxDist >= -2 )
				inRangeFlag = true ;
			if ( inRangeFlag )
			{
